...
SolidsColumnMajorBitmap::SolidsColumnMajorBitmap(int world_width, int world_height) {

    int column_size_in_int_sized_blocks = static_cast<int>(std::ceil(float(world_height) / float(sizeof(unsigned int) * 8)));
...
SolidsRowMajorBitmap::SolidsRowMajorBitmap(int world_width, int world_height) {
    ...
    // Allocate aligned memory
    void* raw_memory = std::aligned_alloc(CACHE_LINE_BYTE_SIZE, world_height * aligned_row_size);
    if (raw_memory == nullptr) { exit(-1); }
...
BlockWorld::BlockWorld(int width, int height) : width_(width), height_(height) {
    scmb_ = new SolidsColumnMajorBitmap(width_, height_);
    srmb_ = new SolidsRowMajorBitmap(width_, height_);
}
...
/*
BlockMatrix::BlockMatrix(int width, int height) : width_(width), height_(height) {
    int block_byte_size = sizeof(Block);
    int column_size = height * block_byte_size;
